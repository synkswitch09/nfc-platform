...
import { isNavigationActive, parseFooterConfig, parseFooterLinks, parseHeaderConfig } from "@/lib/site-chrome";
...
describe("site chrome configuration", () => {
  ...
  it("rejects unsafe navigation and footer links", () => {
    expect(() => parseHeaderConfig({ faqHref: "javascript:alert(1)" })).toThrow();
    expect(() => parseFooterConfig({ customLinks: [{ label: "Unsafe", href: "javascript:alert(1)", visible: true, order: 0 }] })).toThrow();
  });

  it("keeps footer link order and only complete rows", () => {
    expect(parseFooterLinks("About | /about\nBroken\nHelp | https://example.com/help")).toEqual([
      { label: "About", href: "/about", visible: true, order: 0 },
      { label: "Help", href: "https://example.com/help", visible: true, order: 2 },
    ]);
  });
  ...
  it("derives active navigation from the current route", () => {
    ...
    expect(isNavigationActive("/guides", "/#faqs")).toBe(false);
  });
});
