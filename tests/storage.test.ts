...
import { createStorageKey, storageKeyEnvironment } from "@/lib/storage/keys";
...
describe("storage providers", () => {
  it("writes, reads and deletes through the local provider", async () => {
    ...
    const provider = new LocalStorageProvider(directory); const key = createStorageKey("development", "00000000-0000-4000-8000-000000000001", "png");
  ...
  it("uses the Azure provider contract without exposing its credential in headers", async () => {
    ...
    await provider.put(createStorageKey("staging", "00000000-0000-4000-8000-000000000002", "jpg"), Uint8Array.from([4]), { contentType: "image/jpeg", metadata: { environment: "staging" } });
    expect(fetcher).toHaveBeenCalledOnce();
    const [url, request] = fetcher.mock.calls[0];
    expect(String(url)).toContain("staging-00000000-0000-4000-8000-000000000002.jpg?sv=secret");
  ...
  it("prefixes new objects to make environment ownership explicit", () => {
    ...
    expect(storageKeyEnvironment(createStorageKey("development", id, "webp"))).toBe("development");
    expect(storageKeyEnvironment(createStorageKey("production", id, "webp"))).toBe("production");
