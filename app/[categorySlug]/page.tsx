...
export async function generateMetadata({
    ...
    return {
      ...
      openGraph: {
        ...
        images: page.ogImageUrl
          ? [page.ogImageUrl]
          : modularHeroImage(localized.sections)
            ? [modularHeroImage(localized.sections)!]
            : undefined,
      },
    };
  ...
  const localizedSections = localizeSections(
  ...
  return {
    ...
    openGraph: {
      ...
      images: category.ogImageUrl
        ? [category.ogImageUrl]
        : modularHeroImage(localizedSections)
          ? [modularHeroImage(localizedSections)!]
...
export default async function PublicCategoryPage({
  ...
  if (!category) {
    ...
    const [page, categories, products] = await Promise.all([
      ...
      db.productCategory.findMany({
        where: { storeId: store.id, status: "PUBLISHED", showLanding: true },
        select: {
          id: true,
          slug: true,
          name: true,
  ...
  const localizedCategory = {
    ...category,
    name:
      category.contentPage?.translations.find((item) => item.locale === locale)
        ?.name ?? category.name,
    landingSections: localizeSections(
      category.landingSections,
      locale,
      store.defaultLocale,
    ),
  ...
  const faq = localizedCategory.landingSections.length
    ? modularFaq(localizedCategory.landingSections)
    ...
    "@graph": [
      {
        ...
        itemListElement: [
          ...
          },
          {
            ...
            position: 3,
            name: category.name,
            item: `${origin}${categoryPublicPath(category.slug)}`,
          },
        ],
      },
      {
        ...
        url: `${origin}${categoryPublicPath(category.slug)}`,
        mainEntity: {
          "@type": "ItemList",
  ...
  return (
    <>
      ...
      />
      {localizedCategory.landingSections.length ? (
        <LandingSectionRenderer
          category={localizedCategory}
          store={storefront}
        />
      ) : (
        <CategoryLanding category={localizedCategory} store={storefront} />
      )}
    </>
  );
}
