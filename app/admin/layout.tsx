...
const groups = [
  ...
  },
  {
    ...
    links: [
      ...
      { href: "/admin/categories", label: "Categories", icon: Archive },
  ...
  },
  {
    ...
    links: [
      ...
      },
      {
        ...
        icon: ScanLine,
        capability: StoreCapability.NFC,
      },
    ],
  },
  {
    ...
    .map((group) => ({
      ...
      links: group.links.filter(
        (link) =>
          !("capability" in link) ||
          !link.capability ||
          hasStoreCapability(store, link.capability),
      ),
    }))
    .map((group) =>
    ...
    <div className="admin-shell">
      <aside className="admin-sidebar">
        <Link href="/admin" className="admin-brand">
          {store.logoUrl ? (
            ...
            <span>{store.displayName.slice(0, 2).toUpperCase()}</span>
          )}
          <div>
            {store.displayName}
      ...
      <main className="admin-main">
        {environment !== "production" && (
          <div className={`environment-banner ${environment}`} role="status">
            {environment.toUpperCase()} ENVIRONMENT · STORE:{" "}
