...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  const { user, store, isPlatformAdmin } = await requireAdminPageContext();
  ...
  return <div className="admin-shell"><aside className="admin-sidebar"><Link href="/admin" className="admin-brand"><span>{store.displayName.slice(0, 2).toUpperCase()}</span><div>{store.displayName}<small>Operations</small></div></Link><AdminStoreSwitcher current={store} stores={stores} platformAdmin={isPlatformAdmin} />{visibleGroups.map(group => <div className="admin-nav-group" key={group.label}><p>{group.label}</p>{group.links.map(({ href, label, icon: Icon }) => <Link href={href} key={href}><Icon size={18} />{label}</Link>)}</div>)}<Link href="/" className="admin-store-link">← View storefront</Link></aside><main className="admin-main">{environment !== "production" && <div className={`environment-banner ${environment}`} role="status">{environment.toUpperCase()} ENVIRONMENT · STORE: {store.displayName.toUpperCase()}</div>}{children}</main></div>;
}
