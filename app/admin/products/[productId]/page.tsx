...
export default async function EditProductPage({ params }: { params: Promise<{ productId: string }> }) {
  ...
  return <div><div className="admin-heading"><div><p className="admin-kicker">Catalog</p><h1>Edit product</h1><p>{product.name} · updated {product.updatedAt.toLocaleDateString("en-AU")}</p></div></div><ProductImageManager productId={product.id} images={product.images} /><AdminProductForm initial={initial} categories={categories} /></div>;
}
