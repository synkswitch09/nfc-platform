...
export default async function ManufacturingPage() {
  ...
  const [counts, batches] = await Promise.all([
    Promise.all(stages.map(async status => ({ status, count: await db.nFCTag.count({ where: { storeId: store.id, manufacturingStatus: status } }) }))),
    db.manufacturingBatch.findMany({ where: { storeId: store.id }, take: 8, orderBy: { createdAt: "desc" }, include: { product: { select: { name: true } }, _count: { select: { tags: true } } } }),
  ]);
