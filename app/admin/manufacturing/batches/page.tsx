...
export default async function ManufacturingBatchesPage() {
  const [products, batches] = await Promise.all([
    db.product.findMany({ where: { status: { in: ["ACTIVE", "DRAFT"] }, type: { not: "ACCESSORY" } }, select: { id: true, name: true, variants: { where: { active: true }, select: { id: true, name: true, sku: true } } }, orderBy: { name: "asc" } }),
    db.manufacturingBatch.findMany({ take: 100, orderBy: { createdAt: "desc" }, include: { product: { select: { name: true } }, productVariant: { select: { name: true, sku: true } }, createdBy: { select: { name: true } }, _count: { select: { tags: true } } } }),
  ]);
