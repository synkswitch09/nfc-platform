...
export default async function ManufacturingBatchPage({ params }: { params: Promise<{ batchId: string }> }) {
  ...
  const batch = await db.manufacturingBatch.findFirst({ where: { id: batchId, storeId: store.id }, include: { product: true, productVariant: true, createdBy: { select: { name: true } }, tags: { where: { storeId: store.id }, orderBy: { batchSequence: "asc" } } } });
  if (!batch) notFound();
