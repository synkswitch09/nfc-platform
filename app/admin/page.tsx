...
export default async function AdminPage() {
  ...
  const [revenueToday, revenue30, waiting, processing, lowStock, products, customers, activeTags, unclaimedTags, batches, scansToday, scans30, recentOrders, recentActivations] = await Promise.all([
    ...
    db.scanEvent.count({ where: { scannedAt: { gte: today } } }),
    db.scanEvent.count({ where: { scannedAt: { gte: last30 } } }),
    db.order.findMany({ take: 6, orderBy: { createdAt: "desc" }, include: { items: { take: 1 } } }),
    db.tagActivation.findMany({ where: { success: true }, take: 6, orderBy: { attemptedAt: "desc" }, include: { tag: { include: { product: { select: { name: true } }, owner: { select: { name: true } } } } } }),
  ]);
  const metrics = [
