...
export async function GET(request: NextRequest) {
  ...
  await db.$transaction(async tx => {
    ...
    if (record.orderClaimId) {
      const order = await tx.order.findFirst({ where: { id: record.orderClaimId, storeId: store.id, userId: null, claimedAt: null, guestEmail: record.user.email, claimExpiresAt: { gt: new Date() } } });
      if (order) {
        ...
        if (claimed.count === 1) {
          ...
          await tx.auditLog.create({ data: { actorId: record.userId, storeId: store.id, action: "GUEST_ORDER_CLAIMED", entityType: "Order", entityId: order.id } });
        }
      }
    }
  });
