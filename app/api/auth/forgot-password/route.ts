...
export async function POST(request: NextRequest) {
  ...
  if (parsed.success) { const user = await db.user.findUnique({where:{email:parsed.data.email}}); if(user){ const token=createOpaqueToken(); await db.passwordReset.create({data:{userId:user.id,tokenHash:sha256(token),expiresAt:new Date(Date.now()+60*60*1000)}}); const origin=getRuntimeConfig().appUrl; await sendTransactionalEmail({to:user.email,subject:"Reset your Tapkin password",text:`Reset your password: ${origin}/reset-password?token=${token}`}).catch(()=>undefined); } }
