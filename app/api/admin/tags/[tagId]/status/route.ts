...
export async function POST(request: NextRequest, { params }: { params: Promise<{ tagId: string }> }) {
  ...
  const tag = await db.nFCTag.findUnique({ where: { id: tagId }, select: { status: true, ownerId: true } });
  ...
  const changed = await db.nFCTag.updateMany({ where: { id: tagId, status: tag.status }, data: { status: parsed.data.status } });
  if (!changed.count) return jsonError("Tag changed while updating. Refresh and try again.", 409);
  await db.auditLog.create({ data: { actorId: user.id, action: "TAG_STATUS_CHANGED", entityType: "NFCTag", entityId: tagId, metadata: { from: tag.status, to: parsed.data.status, reason: parsed.data.reason } } });
