...
export async function POST(request: NextRequest, { params }: { params: Promise<{ tagId: string }> }) {
  ...
  const tag = await db.nFCTag.findUnique({ where: { id: tagId }, select: { manufacturingStatus: true, manufacturingBatchId: true } });
  ...
  const changed = await db.$transaction(async tx => {
    const updated = await tx.nFCTag.updateMany({ where: { id: tagId, manufacturingStatus: tag.manufacturingStatus }, data: { manufacturingStatus: parsed.data.status, ...(parsed.data.status === "READY" ? { status: "UNCLAIMED" as const } : {}), ...timestamp } });
    if (!updated.count) return false;
    await tx.auditLog.create({ data: { actorId: user.id, action: "TAG_MANUFACTURING_STATUS_CHANGED", entityType: "NFCTag", entityId: tagId, metadata: { from: tag.manufacturingStatus, to: parsed.data.status } } });
    if (tag.manufacturingBatchId) {
      const pending = await tx.nFCTag.count({ where: { manufacturingBatchId: tag.manufacturingBatchId, manufacturingStatus: { not: parsed.data.status } } });
      if (!pending) await tx.manufacturingBatch.update({ where: { id: tag.manufacturingBatchId }, data: { status: parsed.data.status } });
    }
    return true;
  });
