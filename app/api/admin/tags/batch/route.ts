...
export async function POST(request: NextRequest) {
  ...
  if (!hasStoreCapability(store, StoreCapability.NFC)) return jsonError("NFC manufacturing is not enabled for this store", 404);
  ...
  const product = await db.product.findFirst({ where: { id: parsed.data.productId, storeId: store.id }, include: { variants: { where: { active: true } } } });
  ...
  const batch = await db.$transaction(async tx => {
    const created = await tx.manufacturingBatch.create({ data: { batchNumber, storeId: store.id, productId: product.id, productVariantId: variant?.id, quantity: secrets.length, createdById: user.id, notes: parsed.data.notes || null } });
    await tx.nFCTag.createMany({ data: secrets.map(secret => ({ publicTagId: secret.publicTagId, storeId: store.id, activationCodeHash: secret.activationCodeHash, productId: product.id, productVariantId: variant?.id, productType: product.type, status: "MANUFACTURED", manufacturingStatus: "GENERATED", manufacturingBatchId: created.id, batchSequence: secret.sequence })) });
    await tx.auditLog.create({ data: { actorId: user.id, storeId: store.id, action: "MANUFACTURING_BATCH_CREATED", entityType: "ManufacturingBatch", entityId: created.id, metadata: { batchNumber, quantity: secrets.length, productId: product.id } } });
    return created;
  });
