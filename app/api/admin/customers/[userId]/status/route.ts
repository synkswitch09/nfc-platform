...
export async function POST(request: NextRequest, { params }: { params: Promise<{ userId: string }> }) {
  ...
  await db.$transaction([
    ...
    db.auditLog.create({ data: { actorId: actor.id, action: "CUSTOMER_STATUS_CHANGED", entityType: "User", entityId: userId, metadata: { from: target.status, to: parsed.data.status } } }),
  ]);
