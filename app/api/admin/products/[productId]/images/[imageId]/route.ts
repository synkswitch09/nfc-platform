...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ productId: string; imageId: string }> }) {
  ...
  const exists = await db.productImage.findFirst({ where: { id: imageId, productId } }); if (!exists) return jsonError("Image not found", 404);
  await db.$transaction(async tx => {
    ...
    await tx.auditLog.create({ data: { actorId: user.id, action: "PRODUCT_IMAGE_UPDATED", entityType: "ProductImage", entityId: imageId, metadata: { productId } } });
  });
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ productId: string; imageId: string }> }) {
  ...
  const user = await getAdminApiUser(); if (!user) return jsonError("Forbidden", 403);
  const { productId, imageId } = await params;
  const image = await db.productImage.findFirst({ where: { id: imageId, productId } }); if (!image) return jsonError("Image not found", 404);
  await db.$transaction(async tx => {
    ...
    await tx.auditLog.create({ data: { actorId: user.id, action: "PRODUCT_IMAGE_DELETED", entityType: "ProductImage", entityId: imageId, metadata: { productId } } });
  });
