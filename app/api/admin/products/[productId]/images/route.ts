...
export async function POST(request: NextRequest, { params }: { params: Promise<{ productId: string }> }) {
  ...
  try { stored = await validateAndStoreImage(file); } catch (error) { return jsonError(error instanceof Error && error.message === "IMAGE_SIZE" ? "Images must be 5 MB or smaller" : "Use a genuine PNG, JPEG or WebP image", 415); }
  try {
    ...
    const image = await db.$transaction(async tx => {
      ...
      const created = await tx.productImage.create({ data: { productId, storageKey: stored.storageKey, url: `/api/media/${stored.storageKey}`, altText, mimeType: stored.mimeType, byteSize: stored.byteSize, sortOrder: product._count.images, isPrimary: primary } });
