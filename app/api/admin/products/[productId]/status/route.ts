...
export async function POST(request: NextRequest, { params }: { params: Promise<{ productId: string }> }) {
  ...
  const product = await db.product.findUnique({ where: { id: productId }, select: { status: true } }); if (!product) return jsonError("Product not found", 404);
  await db.$transaction([
    ...
    db.auditLog.create({ data: { actorId: user.id, action: parsed.data.status === "HIDDEN" ? "PRODUCT_HIDDEN" : parsed.data.status === "ARCHIVED" ? "PRODUCT_ARCHIVED" : "PRODUCT_STATUS_CHANGED", entityType: "Product", entityId: productId, metadata: { fromStatus: product.status, toStatus: parsed.data.status, reason: parsed.data.reason } } }),
  ]);
