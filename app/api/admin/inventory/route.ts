...
export async function POST(request: NextRequest) {
  ...
  const result = await db.$transaction(async tx => {
    const variant = await tx.productVariant.findUnique({ where: { id: variantId }, select: { inventory: true, reservedInventory: true } });
    ...
    await tx.auditLog.create({ data: { actorId: user.id, action: "INVENTORY_ADJUSTED", entityType: "ProductVariant", entityId: variantId, metadata: { from: variant.inventory, to: quantity, reason } } });
    return { quantity };
  }).catch(error => {
