...
const imageSource = z.string().trim().refine(isSafeImageSource, "Use an uploaded image or an HTTPS image URL").or(z.literal(""));
...
const schema = z.object({
  ...
  logoUrl: imageSource.optional(), faviconUrl: imageSource.optional(), siteTitle: z.string().trim().min(5).max(70), siteDescription: z.string().trim().min(20).max(170), defaultSocialImageUrl: imageSource.optional(),
...
export async function POST(request: NextRequest) {
  ...
  await db.$transaction([
    db.store.update({ where: { id: store.id }, data: { displayName: value.storeName, legalName: value.businessName || null, supportEmail: value.supportEmail, currency: value.currency, country: value.defaultCountry, timezone: value.timezone, logoUrl: value.logoUrl || null, faviconUrl: value.faviconUrl || null, seoTitle: value.siteTitle, seoDescription: value.siteDescription, socialImageUrl: value.defaultSocialImageUrl || null, socialLinks: { instagram: value.instagram || null, facebook: value.facebook || null, tiktok: value.tiktok || null, linkedin: value.linkedin || null }, shippingConfig: { flatRateCents: value.flatRateCents, freeOverCents: value.freeOverCents }, headerConfig: value.headerConfig, footerConfig: value.footerConfig, theme: value.theme, homepage: { ...store.homepage, ...value.homepage }, ...platformData } }),
