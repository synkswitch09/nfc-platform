...
export async function POST(request: NextRequest, { params }: { params: Promise<{ orderId: string }> }) {
  ...
  const order = await db.order.findFirst({ where: { id: orderId, storeId: store.id }, select: { status: true } });
  ...
  const updatedOrder = await db.order.findFirst({ where: { id: orderId, storeId: store.id }, select: { orderNumber: true, storeDisplayName: true, status: true, trackingNumber: true, shippingCarrier: true, guestEmail: true, user: { select: { email: true } } } });
  ...
  if (email && updatedOrder && ["PROCESSING", "READY_TO_SHIP", "SHIPPED", "DELIVERED"].includes(updatedOrder.status)) {
    ...
    await sendTransactionalEmail({ to: email, subject: `${updatedOrder.storeDisplayName} order ${updatedOrder.orderNumber}: ${updatedOrder.status.replaceAll("_", " ")}`, text: `Your ${updatedOrder.storeDisplayName} order is now ${updatedOrder.status.replaceAll("_", " ").toLowerCase()}.${tracking}` }).catch(() => undefined);
  }
