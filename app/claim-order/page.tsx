...
export default async function ClaimOrderPage({ searchParams }: { searchParams: Promise<{ order?: string; claim?: string }> }) {
  ...
  return <section className="auth-shell"><div className="auth-card"><p className="eyebrow">{store.displayName} order {params.order}</p><h1>Manage your {hasStoreCapability(store, StoreCapability.NFC) ? "connected products" : "order"}</h1><p className="muted">Link this guest purchase to a verified account. The order number alone is never enough to claim it.</p>{user ? <ClaimOrderForm orderNumber={params.order} claimToken={params.claim} /> : <div className="actions"><Link className="button" href={`/login?next=${encodeURIComponent(returnTo)}`}>Sign in</Link><Link className="button secondary" href={`/register?order=${encodeURIComponent(params.order)}&claim=${encodeURIComponent(params.claim)}`}>Create account</Link></div>}</div></section>;
}
