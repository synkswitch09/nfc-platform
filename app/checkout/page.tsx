...
export default async function CheckoutPage() {
  ...
  return <section className="section compact-section"><div className="section-head"><p className="eyebrow">Secure checkout · {settings.storeName}</p><h1 className="page-title">Complete your order</h1><p className="lead">Checkout as a guest or use your signed-in account. {nfcEnabled ? "An account is only required later to activate and manage connected products." : "Your order can be linked securely if you create an account later."}</p></div><CheckoutForm account={user ? { name: user.name, email: user.email, address } : null} store={{ displayName: settings.storeName, currency: settings.currency, nfcEnabled }} /></section>;
}
