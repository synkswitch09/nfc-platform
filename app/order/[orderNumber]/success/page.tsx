...
export default async function OrderSuccessPage({ params, searchParams }: { params: Promise<{ orderNumber: string }>; searchParams: Promise<{ token?: string }> }) {
  const [{ orderNumber }, query, user] = await Promise.all([params, searchParams, getCurrentUser()]);
  const tokenHash = query.token ? sha256(query.token) : undefined;
  const order = await db.order.findFirst({
    where: { orderNumber, OR: [user ? { userId: user.id } : { id: "00000000-0000-0000-0000-000000000000" }, tokenHash ? { claimTokenHash: tokenHash, claimExpiresAt: { gt: new Date() } } : { id: "00000000-0000-0000-0000-000000000000" }] },
    include: { items: true },
  });
  if (!order) notFound();
  ...
  return <section className="section compact-section order-success"><ClearCartOnSuccess /><CheckCircle2 size={54} /><p className="eyebrow">Payment received</p><h1 className="page-title">Thank you, {order.customerName?.split(" ")[0] ?? "your order is confirmed"}.</h1><p className="lead">We’ll prepare your personalised product and send updates to {order.guestEmail ?? user?.email}.</p>
    ...
    {!order.userId && claimUrl && <div className="post-purchase"><p className="eyebrow">Next step</p><h2>Create an account to manage your NFC products</h2><p>Your purchase is complete. A free account is required only to activate the tag, edit its profile, change its destination and view scans.</p><div className="actions"><Link className="button" href={registerUrl}>Create account with email</Link><Link className="button secondary" href={claimUrl}>I already have an account</Link></div><p className="fine-print">Google and Apple sign-in will appear here when their external credentials are configured.</p></div>}
