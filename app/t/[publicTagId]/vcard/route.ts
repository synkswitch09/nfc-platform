...
export async function GET(_request: Request, { params }: { params: Promise<{ publicTagId: string }> }) {
  ...
  const tag = await db.nFCTag.findFirst({ where: { publicTagId: publicTagId.toUpperCase(), status: "ACTIVE", productType: "BUSINESS", profile: { isPublic: true } }, include: { profile: { include: { business: true } } } });
