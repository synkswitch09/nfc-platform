...
export async function GET(_request: Request, { params }: { params: Promise<{ publicTagId: string }> }) {
  ...
  const tag = await db.nFCTag.findFirst({ where: { storeId: store.id, publicTagId: publicTagId.toUpperCase(), status: { in: ["ACTIVE", "LOST"] }, productType: "BUSINESS", profile: { isPublic: true } }, include: { profile: { include: { business: true } } } });
