...
export default async function PublicTagPage({ params }: { params: Promise<{ publicTagId: string }> }) {
  ...
  void db.scanEvent.create({ data: { tagId: tag.id, dayBucket, deviceCategory: deviceCategory(h.get("user-agent") ?? ""), countryCode: h.get("cf-ipcountry")?.slice(0,2) || null, region: h.get("cf-region")?.slice(0,80) || null } }).catch(() => undefined);
  if (tag.profile.social?.mode === "DIRECT_REDIRECT") { const url = validRedirect(tag.profile.social.redirectUrl); if (url) redirect(url); }

  const profile = tag.profile; const primary = profile.contacts[0];
  if (profile.pet) return <Shell><Dog size={38} /><p className="eyebrow">Pet safety profile</p><h1 style={{fontSize:"3.2rem"}}>{profile.displayName}</h1>{profile.pet.breed && <p className="lead">{profile.pet.breed}{profile.pet.approximateAge ? ` · ${profile.pet.approximateAge}` : ""}</p>}<PublicActions phone={primary?.phone} />{profile.pet.medicalInfo && <Info title="Important medical information" text={profile.pet.medicalInfo} />}{profile.pet.allergies && <Info title="Allergies" text={profile.pet.allergies} />}{profile.pet.behaviourNotes && <Info title="How to approach me" text={profile.pet.behaviourNotes} />}</Shell>;
  if (profile.child) return <Shell><ShieldCheck size={38} />{profile.child.status === "MISSING" && <div className="notice"><AlertTriangle size={18} /> This child is currently reported missing. Please contact their guardian now.</div>}<p className="eyebrow">Emergency profile</p><h1 style={{fontSize:"3.2rem"}}>{profile.displayName}</h1>{profile.child.approximateAge && <p className="lead">Approximate age: {profile.child.approximateAge}</p>}<PublicActions phone={primary?.phone} label="guardian" />{profile.child.criticalMedicalInfo && <Info title="Critical medical information" text={profile.child.criticalMedicalInfo} />}{profile.child.allergies && <Info title="Allergies" text={profile.child.allergies} />}{profile.child.communicationNotes && <Info title="Communication guidance" text={profile.child.communicationNotes} />}</Shell>;
  if (profile.business) return <Shell><BriefcaseBusiness size={38} /><p className="eyebrow">Digital contact</p><h1 style={{fontSize:"3.2rem"}}>{profile.displayName}</h1><p className="lead">{profile.business.jobTitle}{profile.business.company ? ` at ${profile.business.company}` : ""}</p>{profile.business.bio && <p>{profile.business.bio}</p>}<PublicActions phone={profile.business.phone} label="contact" />{profile.business.email && <a className="button secondary" href={`mailto:${profile.business.email}`}>Email {profile.displayName}</a>}</Shell>;
  if (profile.luggage) return <Shell><Luggage size={38} /><p className="eyebrow">Luggage recovery</p><h1 style={{fontSize:"3.2rem"}}>{profile.displayName}</h1><p className="lead">{profile.luggage.message ?? "Thank you for finding this item."}</p><PublicActions phone={profile.luggage.contactPhone} label="owner" /></Shell>;
  ...
  return <Shell><Radio size={38} /><h1 style={{fontSize:"3.2rem"}}>{profile.displayName}</h1>{profile.social?.bio && <p className="lead">{profile.social.bio}</p>}<div className="form">{links.map(([label,url]) => validRedirect(url) && <a className="button secondary" href={url} key={label} rel="noopener noreferrer">{label}</a>)}</div></Shell>;
}
