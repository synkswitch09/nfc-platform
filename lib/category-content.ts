...
const useCaseSchema = z.object({ icon: z.string(), title: z.string(), description: z.string(), order: z.number(), visible: z.boolean() });
...
export const allCategoryUseCases = (value: unknown) => parseList(useCaseSchema, value);
...
export const categoryUseCases = (value: unknown) => allCategoryUseCases(value).filter(item => item.visible);
...
export type CategoryUseCase = z.infer<typeof useCaseSchema>;
