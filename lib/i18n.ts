...
export function resolveLocale(
  ...
  return (
    ...
    (available.includes(normaliseLocale(fallback))
...
export function compactLocaleName(locale: string) {
  const [language = locale, region] = normaliseLocale(locale).split("-");
  const languageName =
  ...
  return region ? `${languageName} (${region.toUpperCase()})` : languageName;
}
...
export function languageAlternates(
  ...
  return Object.fromEntries(
    locales.map((locale) => [
      locale,
      `${origin}${localizedPath(path, locale, defaultLocale)}`,
    ]),
  );
}
...
export function localizeContentPage<
  TSection extends {
    id: string;
    content: unknown;
    translations?: Array<{ locale: string; content: unknown }>;
  },
  TPage extends {
    name: string;
...
>(page: TPage, locale: string, defaultLocale: string) {
  ...
  const translation = page.translations?.find(
    (item) => normaliseLocale(item.locale) === normaliseLocale(locale),
  );
  return {
    ...page,
...
>(sections: TSection[], locale: string, defaultLocale: string) {
  if (normaliseLocale(locale) === normaliseLocale(defaultLocale))
    return sections;
