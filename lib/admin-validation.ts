...
const reservedCategorySlugs = new Set(["activate", "admin", "api", "cart", "categories", "checkout", "claim-order", "dashboard", "faq", "forgot-password", "guides", "login", "order", "privacy", "products", "register", "reset-password", "shop", "t", "terms", "verify-email"]);
...
export const adminProductSchema = z.object({
  ...
  hsCode: z.string().trim().regex(/^[0-9]{4,10}$/, "Use a 4–10 digit HS code").nullable().optional(),
  customsValueCents: z.number().int().min(0).max(100_000_000).nullable().optional(),
  dutiesHandling: z.enum(["UNDETERMINED", "RECIPIENT_PAYS", "SENDER_PAYS"]).default("UNDETERMINED"),
  restrictedItem: z.boolean().default(false),
  seoTitle: z.string().trim().max(70).nullable().optional(),
  seoDescription: z.string().trim().max(170).nullable().optional(),
  ogImageUrl: optionalImageSource,
...
export const adminCategorySchema = z.object({
  ...
  cardImageUrl: optionalImageSource,
  ...
  heroImageUrl: optionalImageSource,
  heroImageAlt: z.string().trim().max(160).nullable().optional(),
  secondaryImageUrl: optionalImageSource,
  ...
  howItWorks: z.array(z.object({ title: z.string().trim().min(2).max(100), description: z.string().trim().min(5).max(500), imageUrl: optionalImageSource, ctaLabel: z.string().trim().max(60).nullable().optional(), ctaHref: optionalInternalPath, order: z.number().int().min(0).max(100), visible: z.boolean() })).max(10).default([]),
  contentSections: z.array(z.object({ layout: z.enum(["IMAGE_LEFT", "IMAGE_RIGHT", "TEXT_ONLY"]), eyebrow: z.string().trim().max(80).nullable().optional(), heading: z.string().trim().min(2).max(160), copy: z.string().trim().min(10).max(2_000), bulletPoints: z.array(z.string().trim().min(2).max(200)).max(10).default([]), imageUrl: optionalImageSource, ctaLabel: z.string().trim().max(60).nullable().optional(), ctaHref: optionalInternalPath, order: z.number().int().min(0).max(100), visible: z.boolean() })).max(12).default([]),
