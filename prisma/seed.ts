...
const swatchColours: Record<string, string> = { black: "#111111", white: "#ffffff", ocean: "#167d9a", coral: "#ee6c4d", lime: "#b8df57", navy: "#14213d", sand: "#d8c3a5", forest: "#315c49", charcoal: "#3f4548" };
...
async function seedHomeDemo() {
  ...
  for (const item of demoProducts) {
    ...
    const productRecord = await db.product.upsert({ where: { storeId_slug: { storeId: store.id, slug: item.slug } }, update: { name: item.name, description: item.description, shortDescription: item.description, categoryId, status: "ACTIVE", shopVisible: true, featured: item.featured, brand: store.displayName, personalisationMode: personalisationMode(item.options) }, create: { storeId: store.id, slug: item.slug, name: item.name, description: item.description, shortDescription: item.description, type: item.type, categoryId, status: "ACTIVE", shopVisible: true, featured: item.featured, brand: store.displayName, personalisationMode: personalisationMode(item.options) } });
    await db.productVariant.upsert({ where: { sku: item.sku }, update: { productId: productRecord.id, name: "Standard", priceCents: item.priceCents, active: true, isDefault: true }, create: { productId: productRecord.id, sku: item.sku, name: "Standard", priceCents: item.priceCents, inventory: 40, isDefault: true } });
