...
type ProfileData = { id: string; type: "PET" | "CHILD" | "SOCIAL" | "BUSINESS" | "LUGGAGE"; displayName: string; details: Record<string, unknown>; contacts: Array<{ name: string; relationship?: string | null; phone: string }> };
...
export function ProfileEditor({ profile }: { profile: ProfileData }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (profile.type === "CHILD") details.status = values.status || "NORMAL";
    if (profile.type === "SOCIAL") { details.mode = values.mode || "MULTI_LINK"; details.links = profile.details.links ?? {}; }
  ...
  return <form className="form card" onSubmit={submit}>
    ...
    {profile.type === "SOCIAL" && <label className="field">Mode<select name="mode" defaultValue={String(profile.details.mode ?? "MULTI_LINK")}><option value="MULTI_LINK">Multi-link profile</option><option value="DIRECT_REDIRECT">Direct redirect</option></select></label>}
