...
export function CartView() {
  ...
  if (!lines.length) return <div className="card cart-empty"><ShoppingBag size={42} /><h2>Your cart is empty</h2><p className="muted">Choose a smart product and personalise it before checkout.</p><Link href="/shop" className="button">Browse products</Link></div>;
  ...
  return <div className="cart-layout">
    <div className="cart-lines">{lines.map(line => <article className="cart-row" key={line.key}>
      <div className="cart-thumb"><RadioMark /></div>
...
function RadioMark() { return <span aria-hidden="true">NFC</span>; }
