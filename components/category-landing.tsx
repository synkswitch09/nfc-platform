...
import { ArrowRight, Check, QrCode, Radio } from "lucide-react";
...
export function CategoryLanding({ category }: { category: PublicCategory }) {
  ...
  const sections: Record<string, React.ReactNode> = {
    ...
    products: <section className="category-products" key="products"><SectionHeading eyebrow="Made to be yours" title={`Explore the ${category.name} collection.`} /><div className="shop-grid">{products.map(product => <article className="card product-card" key={product.id}>{product.images[0] ? <Image className="category-product-image" src={product.images[0].url} alt={product.images[0].altText} width={500} height={500} unoptimized /> : <div className="category-product-placeholder"><CategoryIcon name={category.icon} size={34} /><span>Made to order</span></div>}<h3>{product.name}</h3><p>{product.shortDescription ?? product.description}</p>{product.variants[0] && <strong className="price">From {money.format(product.variants[0].priceCents / 100)}</strong>}<Link className="button" href={`/products/${product.slug}`}>View product</Link></article>)}</div>{!products.length && <div className="category-unavailable"><strong>This collection is not accepting new orders right now.</strong><p>Existing Tapkin products and their digital profiles continue to work normally.</p></div>}<Link className="category-shop-link" href={shopHref}>View the full filtered collection <ArrowRight size={17} /></Link></section>,
  ...
  return <section className="category-landing" data-theme={category.visualTheme.toLowerCase()} data-layout={category.landingLayout.toLowerCase()}>
    ...
    <div className="category-landing-hero"><div className="category-hero-copy"><span className="eyebrow"><CategoryIcon name={category.icon} size={15} /> {category.heroEyebrow ?? "Tapkin smart collection"}</span><h1>{category.heroHeadline ?? category.name}</h1><p className="lead">{category.heroDescription ?? category.shortDescription ?? category.description}</p><div className="actions"><Link className="button category-primary" href={category.ctaHref || shopHref}>{category.ctaLabel ?? `Shop ${category.name}`} <ArrowRight size={17} /></Link>{category.secondaryCtaLabel && category.secondaryCtaHref && <Link className="button secondary" href={category.secondaryCtaHref}>{category.secondaryCtaLabel}</Link>}</div><div className="category-hero-trust"><span><Radio size={16} /> NFC</span><span><QrCode size={16} /> QR</span><span><Check size={16} /> No app needed</span></div></div>
      {category.heroImageUrl ? <Image src={category.heroImageUrl} alt={category.heroImageAlt || `${category.name} smart products`} width={900} height={720} priority unoptimized /> : <VisualPlaceholder icon={category.icon} label={category.name} />}
    </div>
...
function VisualPlaceholder({ icon, label, compact = false }: { icon?: string | null; label: string; compact?: boolean }) {
  return <div className={`category-hero-placeholder${compact ? " compact" : ""}`} role="img" aria-label={`${label} collection visual placeholder`}><span className="visual-orbit orbit-one" /><span className="visual-orbit orbit-two" /><div className="smart-object"><CategoryIcon name={icon} size={compact ? 42 : 62} /><strong>{label}</strong><small>Tapkin · NFC + QR</small></div><div className="visual-note">Personalised<br />Made useful.</div></div>;
}
