...
export function CheckoutForm({ account }: { account: { name: string; email: string } | null }) {
  const cart = useCart();
  ...
  const shipping = subtotal >= 6000 ? 0 : 900;
  ...
  return <form className="checkout-layout" onSubmit={submit}>
    <div className="checkout-fields">
      ...
      <section className="checkout-section"><h2>Contact</h2><div className="field-grid"><label className="field">Full name<input name="name" autoComplete="name" defaultValue={account?.name} required /></label><label className="field">Email<input name="email" type="email" autoComplete="email" defaultValue={account?.email} required /></label></div></section>
      <section className="checkout-section"><h2>Delivery address</h2><label className="field">Address<input name="line1" autoComplete="address-line1" required /></label><label className="field">Apartment, suite or unit <span className="optional">Optional</span><input name="line2" autoComplete="address-line2" /></label><div className="field-grid three"><label className="field">Suburb<input name="suburb" autoComplete="address-level2" required /></label><label className="field">State<select name="state" autoComplete="address-level1" defaultValue="SA">{states.map(state => <option key={state}>{state}</option>)}</select></label><label className="field">Postcode<input name="postcode" inputMode="numeric" pattern="[0-9]{4}" maxLength={4} autoComplete="postal-code" required /></label></div></section>
