...
export function CheckoutForm({ account, shippingConfig, store }: { account: { name: string; email: string; address: CheckoutAddress | null } | null; shippingConfig: { flatRateCents: number; freeOverCents: number }; store: { displayName: string; currency: string; nfcEnabled: boolean } }) {
  const cart = useCart();
