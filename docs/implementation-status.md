...
- Commercial catalogue with independent category/product lifecycles, duplicate-as-draft, variants, inventory, pricing, colour swatches, colour/variant-specific galleries, explicit Basic/Personalised modes, safe-delete rules, SEO preview and content-verified product media
...
- Server-authoritative Australian checkout quotes with product/variant dimensions, Store packaging/zones, manual fallback rates and an Australia Post/eParcel adapter boundary
- Ready-to-ship fulfilment with idempotent shipment/label creation, tracking lifecycle, private labels, durable Print Queue, authenticated Tapkin Print Agent and paid-order notification
- Modular Store-scoped landing CMS with typed add/edit/hide/show/reorder/duplicate/delete operations and migrated Tapkin category experiences
- AUD/GST presentation, metadata, canonical URLs, robots, sitemap, editorial guides, and Organization, WebSite, Product, Offer, Breadcrumb, ItemList, Article and legitimate FAQ structured data
...
- Live carrier APIs and Stripe refund initiation. Manual/MyPost Business rates and mock development labels are implemented; a real Australia Post Shipping and Tracking adapter remains gated on approved eParcel credentials and staging certification. Operations cannot mark a refund or manufacture an unpaid order without a real external action.
