...
3. In **Admin → Etsy**, connect the seller account, save the Etsy taxonomy and shipping defaults, then create a draft from a Tapkin product. The flow uploads its images and creates up to three custom variations (Colour, Size and Style) with the local SKUs. You may explicitly publish after Etsy accepts the draft. Existing Etsy listings can still be linked only when all active local SKUs match exactly.
...
Credentials and OAuth tokens never reach the browser; tokens are encrypted at rest using the deployment session secret. Each scheduled run updates quantity, price, enabled state, title and description for explicitly linked listings, and imports paid, non-cancelled Etsy receipts as paid Tapkin orders. Etsy inventory retains its own taxonomy property IDs, while Tapkin verifies that an Etsy Colour/Color, Size or Style property uses the same selected variant value before it changes stock. An imported receipt is recorded exactly once, decrements the matching local SKU, creates its manufacturing jobs, and triggers the paid-order notification. It stops safely if a receipt contains an unlinked listing, an unknown SKU, mismatched variation, currency mismatch, or insufficient no-backorder stock; correct the mapping/stock and run the sync again.

The integration creates drafts only after an administrator has configured the seller-specific taxonomy, shipping and processing-profile decisions in Admin. It does not yet mark Etsy receipts as shipped or push Tapkin tracking back to Etsy. The Etsy API itself requires an app API key on every request plus OAuth/PKCE for seller-authorised writes. [Etsy authentication documentation](https://developers.etsy.com/documentation/essentials/authentication/) and [Etsy listing documentation](https://developer.etsy.com/documentation/tutorials/listings/) describe those requirements.
